...
do
  ...
  #curl $VERBOSITY --fail --continue-at - "$TARBALL_URL" --output "$TARBALL_FILE"

  #aria2c -j16 -x16 -s100 -V --dir=$HOME/Downloads --out=meteor-bootstrap-$PLATFORM-$RELEASE.tar.gz $TARBALL_URL && cp $HOME/Downloads/meteor*.gz $TARBALL_FILE

  ln -s -T ${HOME}/Downloads/meteor-boot*.gz ${TARBALL_FILE}
